...
resource "null_resource" "build_Report" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/Report/build.sh"
  }
}
...
data "archive_file" "Report" {
  type        = "zip"
  output_path = "${path.module}/../code/Report.zip" # TODO: change here
  source_dir  = "${path.module}/../code/Report"     # TODO: change here

  depends_on = [
    null_resource.build_Report # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.Report.
...
resource "aws_lambda_function" "Report" {
  ...
  function_name = "${var.group_name}_${terraform.workspace}_Report" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.Report.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "Report" {
  ...
  function_name = aws_lambda_function.Report.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/${terraform.workspace}/${var.group_name}/Report" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "Report" {
  ...
  integration_uri    = aws_lambda_function.Report.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "Report" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/Report" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.Report.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "Report_log" {
  name              = "/aws/lambda/${aws_lambda_function.Report.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
