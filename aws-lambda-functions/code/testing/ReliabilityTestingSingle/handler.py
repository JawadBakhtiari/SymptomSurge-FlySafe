...
def return_report(all_endpoints, team_name):
    filename = f"{team_name}_Reliability_Test_Single_Endpoint.pdf"
    i = 1
    s3 = boto3.client('s3')
    bucket = 'se3011-symptom-surge-bucket'
    filename = f"{team_name}_Reliability_Test_Single_Endpoint.pdf"
    try:
        while s3.get_object(Bucket=bucket, Key=f"{team_name}_Reliability_Test_Single_Endpoint_{i}.pdf").load():
            i += 1
        filename = f"{team_name}_Reliability_Test_Single_Endpoint_{i}.pdf"
...
def handler(event, context):
    ...
    try:
        logInfo(json.dumps(f"Beginning Reliability Testing on {team_name}'s endpoint!"))
        try:
        ...
        logInfo(json.dumps(f"Successfully performed Reliability Testing on endpoint!"))
        return {
            "statusCode": 200,
