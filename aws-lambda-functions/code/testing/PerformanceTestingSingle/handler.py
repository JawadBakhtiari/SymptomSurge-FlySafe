...
def return_report(all_endpoints, team_name):
    filename = f"{team_name}_Performance_Test_Single_Endpoint.pdf"
    i = 1
    s3 = boto3.client('s3')
    bucket = 'se3011-symptom-surge-bucket'
    filename = f"{team_name}_Performance_Test_Single_Endpoint.pdf"
    try:
        while s3.get_object(Bucket=bucket, Key=f"{team_name}_Performance_Test_Single_Endpoint_{i}.pdf").load():
            i += 1
        filename = f"{team_name}_Performance_Test_Single_Endpoint_{i}.pdf"
...
def handler(event, context):
    ...
    if (params.get("teamName") == None):
        ...
        info = symptom_surge_endpoints[endpoint]
        team_name = "Symptom Surge"
    else:
        info = event["queryStringParameters"]["endpoint"]
        team_name = event["queryStringParameters"]["teamName"]
    try:
        logInfo(json.dumps(f"Beginning Performance Testing on {team_name}'s endpoint!"))
        try:
            result_dict = (test_api(team_name, info["url"], info["parameters"], info["method"]))
            time_taken = result_dict[team_name]
            results[team_name] = time_taken
        except Exception as e:
            message = f"Error occured with {team_name} API: {e}"
            logError(message)
        pdf_link = return_report(results, team_name)
        logInfo(json.dumps(f"Successfully performed Performance Testing on endpoint!"))
        return {
            "statusCode": 200,
