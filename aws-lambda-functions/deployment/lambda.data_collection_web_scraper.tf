...
resource "null_resource" "build_data_collection_web_scraper" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/data_collection_web_scraper/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "data_collection_web_scraper" {
  type        = "zip"
  output_path = "${path.module}/../code/data_collection_web_scraper.zip" # TODO: change here
  source_dir  = "${path.module}/../code/data_collection_web_scraper"     # TODO: change here

  depends_on = [
    null_resource.build_data_collection_web_scraper # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.data_collection_web_scraper.
...
resource "aws_lambda_function" "data_collection_web_scraper" {
  filename      = data.archive_file.data_collection_web_scraper.output_path
  function_name = "${var.group_name}_${terraform.workspace}_data_collection_web_scraper" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.data_collection_web_scraper.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "data_collection_web_scraper" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.data_collection_web_scraper.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/${terraform.workspace}/${var.group_name}/data_collection_web_scraper" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "data_collection_web_scraper" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.data_collection_web_scraper.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "data_collection_web_scraper" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/data_collection_web_scraper" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.data_collection_web_scraper.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "data_collection_web_scraper_log" {
  name              = "/aws/lambda/${aws_lambda_function.data_collection_web_scraper.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
