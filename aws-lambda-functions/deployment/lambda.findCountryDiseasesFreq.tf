...
resource "null_resource" "build_findCountryDiseasesFreq" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/findCountryDiseasesFreq/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "findCountryDiseasesFreq" {
  type        = "zip"
  output_path = "${path.module}/../code/findCountryDiseasesFreq.zip" # TODO: change here
  source_dir  = "${path.module}/../code/findCountryDiseasesFreq"     # TODO: change here

  depends_on = [
    null_resource.build_findCountryDiseasesFreq # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.findCountryDiseasesFreq.
...
resource "aws_lambda_function" "findCountryDiseasesFreq" {
  filename      = data.archive_file.findCountryDiseasesFreq.output_path
  function_name = "${var.group_name}_${terraform.workspace}_findCountryDiseasesFreq" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.findCountryDiseasesFreq.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "findCountryDiseasesFreq" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.findCountryDiseasesFreq.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/${terraform.workspace}/${var.group_name}/findCountryDiseasesFreq" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "findCountryDiseasesFreq" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.findCountryDiseasesFreq.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "findCountryDiseasesFreq" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/findCountryDiseasesFreq" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.findCountryDiseasesFreq.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "findCountryDiseasesFreq_log" {
  name              = "/aws/lambda/${aws_lambda_function.findCountryDiseasesFreq.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
